...
# mozjpeg library naming: macOS uses libjpeg.62.dylib, Linux uses libjpeg.so.62
$(eval $(call target,LIBJPEG62,LIBMOZJPEG,$(if $(IS_DARWIN),libjpeg.62$(DLEXT),libjpeg$(DLEXT).62)))
...
test :
	...
	$(call check_lib,$(if $(IS_DARWIN),libjpeg.62$(DLEXT),libjpeg$(DLEXT).62))
